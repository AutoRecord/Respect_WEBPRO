...
出力ファイル (22テーブル):
    - buildings.csv              建物マスタ（様式0）
    - all_rooms.csv              室仕様（様式1）
    - all_zones.csv              空調ゾーン（様式2-1）
    - all_wall_specs.csv         外壁仕様・親（様式2-2）
    - all_wall_layers.csv        外壁層・子（様式2-2）
    - all_window_specs.csv       窓仕様（様式2-3）
    - all_envelopes.csv          外皮（様式2-4）
    - all_heat_source_groups.csv 熱源群・親（様式2-5）
    - all_heat_source_units.csv  熱源ユニット・子（様式2-5）
    - all_pump_groups.csv        二次ポンプ群・親（様式2-6）
    - all_pump_units.csv         二次ポンプユニット・子（様式2-6）
    - all_ahu_groups.csv         空調機群・親（様式2-7）
    - all_ahu_units.csv          空調機ユニット・子（様式2-7）
    - all_vent_rooms.csv         換気室（様式3-1）
    - all_vent_fans.csv          換気送風機（様式3-2）
    - all_lighting.csv           照明（様式4）
    - all_hw_rooms.csv           給湯室（様式5-1）
    - all_hw_equipment.csv       給湯機器（様式5-2）
    - all_elevators.csv          昇降機（様式6）
    - all_pv.csv                 太陽光発電（様式7-1）
    - all_cogen.csv              コージェネレーション（様式7-3）
    - all_non_ac_envelope.csv    非空調外皮（様式8）
"""
...
def extract_building_info(wb, building_id):
    ...
    for row in ws.iter_rows(min_row=1, max_row=25, max_col=7, values_only=True):
        label = row[1] if row[1] else ''
        value = row[2]
        
        if label == 'シート作成月日':
            info['sheet_date'] = value
        elif label == '入力責任者':
            info['input_person'] = value
        elif label == '評価対象':
            info['evaluation_target'] = value
        elif label == '建物の名称':
            info['name'] = value
        elif label == '建築物所在地':
            info['prefecture'] = row[3]  # 都道府県の値
            info['city'] = row[5] if len(row) > 5 else None  # 市区町村の値
        elif label == '省エネ基準地域区分':
            info['region'] = value
        elif label == '構造':
            info['structure'] = value
        elif label == '階数':
            info['floors_above'] = row[3]  # 地上
            info['floors_below'] = row[5] if len(row) > 5 else None  # 地下
        elif label == '敷地面積 [㎡]':
            info['site_area'] = value
        elif label == '建築面積 [㎡]':
            info['building_area'] = value
        elif label == '延べ面積 [㎡]':
            info['total_area'] = value
        elif label == '年間日射地域区分':
            info['solar_region'] = value
        elif '一次エネルギー換算値（冷熱）' in str(label):
            info['external_heat_cool'] = value
        elif '一次エネルギー換算値（温熱）' in str(label):
            info['external_heat_heat'] = value
...
# ============================================================
# 様式8: 非空調外皮（空白セル継承パターン）
# ============================================================

def convert_non_ac_envelope(wb, building_id):
    """非空調外皮シートを変換（ゾーン情報の空白継承）"""
    ws = wb['8) 非空調外皮']
    records = []
    counter = 1
    current_zone = None
